...
void LCD_PutChar(unsigned int x,unsigned int y,unsigned short *c)
{
	unsigned int idx = 0;
	LCD_SetX(x,16);
	LCD_SetY(y,24);
...
{
	c-=32;
	LCD_PutChar(x,y,&ASCII_Table[c*24]);
}

void LCD_displayString(unsigned int y,char *s)
{
	unsigned int i = 0, refcolumn = 0;
	while((*s!= 0) & (i<20))
	{
		LCD_displayChar(refcolumn,y,*s);
		refcolumn += 16;
		s++;i++;
	}
}
